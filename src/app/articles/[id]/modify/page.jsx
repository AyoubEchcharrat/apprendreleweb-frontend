...
export default function Modify() {
	...
	useEffect(() => {
		fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/articles/${currentArticleId}`)
			.then((res) => res.json())
			.then((article) => {
				setData(article);
				setLoading(false);
	...
	const handleDelete = async () => {
		deleteArticle(userToken, currentArticleId).then(() => {
			window.location.assign("/articles");
		});
	};
				...
				<div>
					<form className="form" onSubmit={handleSubmit}>
						...
						<div>
							...
							<button className="button-delete_form" onClick={() => handleDelete()}>
								SUPPRIMER L&apos;ARTICLE
							</button>
						</div>
					</form>
